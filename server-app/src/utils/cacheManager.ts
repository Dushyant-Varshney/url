import { getRedisClient, isRedisAvailable } from '../config/redisConfig';
 ...
 */
const getCacheKey = (shortUrl: string): string => `${CACHE_KEY_PREFIX}${shortUrl}`;
 ...
 */
export const getFromCache = async (shortUrl: string): Promise<string | null> => {
    if (!isRedisAvailable()) {
        return null;
    }
 ...
 * Set value in cache
 * @param shortUrl - The short URL code
 * @param fullUrl - The full URL to cache
 ...
 * @returns Promise<boolean> - Success status
 */
export const deleteFromCache = async (shortUrl: string): Promise<boolean> => {
    if (!isRedisAvailable()) {
        return false;
    }
